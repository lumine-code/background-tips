...
function bindingFor(command, selector) {
  ...
  const bindings = lumine.keymaps.findKeyBindings({ command: command.trim() });
  let binding;
  if (selector) {
...
module.exports = class BackgroundTipsElement {
  constructor() {
    ...
    this.index = -1;
    this.workspaceCenter = lumine.workspace.getCenter();
  ...
  attach() {
    ...
    const [pane] = this.workspaceCenter.getTiledPanes();
    if (!pane) return;
    const paneView = lumine.views.getView(pane);
  ...
  shouldBeAttached() {
    const panes = this.workspaceCenter.getTiledPanes();
    return panes.length === 1 && panes[0].getActiveItem() == null;
  }
  ...
  showNextTip() {
    ...
    for (let i = 0; i < this.tips.length; i++) {
      ...
      if (lumine.packages.isPackageDisabled(tip.packageName)) continue;
  ...
  addPackageTips(pkg) {
    ...
    for (const source of raw) {
      ...
      this.tips.push({ source, packageName: pkg.name });
    }
  ...
  renderTip(tip) {
    ...
    if (!Object.hasOwn(tip, "template")) {
      try {
        tip.template = engine.parse(tip.source);
      } catch (error) {
        tip.template = null;
        // The tip belongs to the package, not to the user, so there is nothing
        // for a notification to ask them to do.
        console.warn(`background-tips: ${tip.packageName} ships an unparsable tip`, error);
      }
    }
    if (tip.template == null) return null;
    ...
    let html;
    try {
      html = engine.renderSync(tip.template, { keys, platform: process.platform });
    } catch (error) {
